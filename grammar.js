module.exports = grammar({
  ...
  rules: {
    ...
    keyword_varchar: ($) =>
      choice(make_keyword('varchar'), seq(make_keyword('character'), $.keyword_varying)),
    ...
    bit: ($) =>
      choice(
        $.keyword_bit,
        seq($.keyword_bit, prec(0, parametric_type($, $.keyword_varying, ['precision']))),
    ...
    _include_time_zone: ($) =>
      seq(choice($.keyword_with, $.keyword_without), $.keyword_time, $.keyword_zone),
    datetimeoffset: ($) => parametric_type($, $.keyword_datetimeoffset),
    time: ($) => seq(parametric_type($, $.keyword_time), optional($._include_time_zone)),
    timestamp: ($) => seq(parametric_type($, $.keyword_timestamp), optional($._include_time_zone)),
    timestamptz: ($) => parametric_type($, $.keyword_timestamptz),

    enum: ($) =>
      seq($.keyword_enum, paren_list(field('value', alias($._literal_string, $.literal)), true)),

    array: ($) =>
      seq(
        $.keyword_array,
        choice(seq('[', comma_list($._expression), ']'), seq('(', $._dml_read, ')'))
      ),
    ...
    block: ($) => seq($.keyword_begin, optional(';'), repeat(seq($.statement, ';')), $.keyword_end),

    statement: ($) =>
      seq(
        optional(seq($.keyword_explain, optional($.keyword_analyze), optional($.keyword_verbose))),
        choice($._ddl_statement, $._dml_write, optional_parenthesis($._dml_read))
      ),
    ...
    _cte: ($) => seq($.keyword_with, optional($.keyword_recursive), $.cte, repeat(seq(',', $.cte))),
    ...
    cte: ($) =>
      seq(
        ...
        wrapped_in_parenthesis(alias(choice($._dml_read, $._dml_write), $.statement))
      ),
    ...
    _select_statement: ($) => optional_parenthesis(seq($.select, optional($.from))),
    ...
    select: ($) => seq($.keyword_select, seq(optional($.keyword_distinct), $.select_expression)),

    select_expression: ($) => seq($.term, repeat(seq(',', $.term))),

    term: ($) => seq(field('value', choice($.all_fields, $._expression)), optional($._alias)),
    ...
    _delete_statement: ($) => seq($.delete, alias($._delete_from, $.from), optional($.returning)),
    ...
    storage_parameters: ($) =>
      seq($.keyword_with, paren_list(seq($.identifier, optional(seq('=', $.literal))), true)),
    ...
    function_body: ($) =>
      choice(
        ...
        seq($.keyword_as, alias(choice($._single_quote_string, $._double_quote_string), $.literal)),
        seq(
          $.keyword_as,
          alias($._dollar_quoted_string_start_tag, $.dollar_quote),
    ...
    function_leakproof: ($) => seq(optional($.keyword_not), $.keyword_leakproof),
    ...
    function_safety: ($) =>
      seq($.keyword_parallel, choice($.keyword_safe, $.keyword_unsafe, $.keyword_restricted)),
    ...
    _operator_class: ($) =>
      seq(
        ...
        optional(field('opclass_parameters', wrapped_in_parenthesis(comma_list($.term))))
      ),
    ...
    create_sequence: ($) =>
      seq(
        $.keyword_create,
        optional(choice(choice($.keyword_temporary, $.keyword_temp), $.keyword_unlogged)),
        $.keyword_sequence,
        optional($._if_not_exists),
        $.object_reference,
        repeat(
        ...
        optional(
          seq(
            choice(
              ...
              seq(optional(seq($.keyword_as, $.keyword_range)), paren_list($._with_settings))
            )
          )
        )
      ),
    ...
    enum_elements: ($) =>
      seq(paren_list(field('enum_element', alias($._literal_string, $.literal)))),
    ...
    add_constraint: ($) =>
      seq($.keyword_add, optional($.keyword_constraint), $.identifier, $.constraint),
    ...
    alter_column: ($) =>
      seq(
        ...
        choice(
          ...
          seq(optional(seq($.keyword_set, $.keyword_data)), $.keyword_type, field('type', $._type)),
          seq(
            $.keyword_set,
            choice(
    ...
    column_position: ($) =>
      choice($.keyword_first, seq($.keyword_after, field('col_name', $.identifier))),
    ...
    alter_database: ($) =>
      seq(
        ...
        choice(
          ...
          seq($.keyword_set, choice(seq($.keyword_tablespace, $.identifier), $.set_configuration))
        )
      ),
        ...
        choice(
          ...
          seq(
            choice(
              ...
              seq($.keyword_drop, $.keyword_attribute, optional($._if_exists), $.identifier),
              seq(
                $.keyword_alter,
                $.keyword_attribute,
                $.identifier,
    ...
    object_reference: ($) =>
      seq(optional(seq(field('schema', $.identifier), '.')), field('name', $.identifier)),
    ...
    insert: ($) =>
      seq(
        ...
        optional(choice($.keyword_low_priority, $.keyword_delayed, $.keyword_high_priority)),
        ...
        optional(
          seq(
            ...
            seq(
              $.keyword_do,
              choice($.keyword_nothing, seq($.keyword_update, $._set_values, optional($.where)))
            )
          )
        )
      ),
    ...
    _partition_spec: ($) => seq($.keyword_partition, paren_list($.table_option, true)),
    ...
    _mysql_update_statement: ($) =>
      prec(0, seq(comma_list($.relation, true), repeat($.join), $._set_values, optional($.where))),

    _postgres_update_statement: ($) => prec(1, seq($.relation, $._set_values, optional($.from))),
    ...
    table_sort: ($) => seq($.keyword_sort, $.keyword_by, paren_list($.identifier, true)),
    ...
    _key_value_pair: ($) =>
      seq(field('key', $.identifier), '=', field('value', alias($._literal_string, $.literal))),
    ...
    assignment: ($) =>
      seq(field('left', alias($._qualified_field, $.field)), '=', field('right', $._expression)),
    ...
    column_definitions: ($) =>
      seq('(', comma_list($.column_definition, true), optional($.constraints), ')'),

    column_definition: ($) =>
      seq(field('name', $._column), field('type', $._type), repeat($._column_constraint)),
    ...
    _column_constraint: ($) =>
      prec.left(
        choice(
          ...
          $._column_comment,
          $._check_constraint,
          seq(optional(seq($.keyword_generated, $.keyword_always)), $.keyword_as, $._expression),
    ...
    cast: ($) =>
      seq(
        ...
        wrapped_in_parenthesis(seq(field('parameter', $._expression), $.keyword_as, $._type))
      ),

    filter_expression: ($) => seq($.keyword_filter, wrapped_in_parenthesis($.where)),
      ...
      prec(
        ...
        seq(
          ...
          choice(
            // default invocation
            paren_list(
              seq(optional($.keyword_distinct), field('parameter', $.term), optional($.order_by))
            ),
    ...
    partition_by: ($) => seq($.keyword_partition, $.keyword_by, comma_list($._expression, true)),
    ...
    window_specification: ($) =>
      wrapped_in_parenthesis(
        seq(optional($.partition_by), optional($.order_by), optional($.window_frame))
      ),

    window_function: ($) =>
      seq($.invocation, $.keyword_over, choice($.identifier, $.window_specification)),
    ...
    relation: ($) =>
      prec.right(
        seq(
          choice($.subquery, $.invocation, $.object_reference, wrapped_in_parenthesis($.values)),
          optional(seq($._alias, optional(alias($._column_list, $.list))))
        )
      ),

    values: ($) => seq($.keyword_values, $.list, optional(repeat(seq(',', $.list)))),
    ...
    group_by: ($) =>
      seq($.keyword_group, $.keyword_by, comma_list($._expression, true), optional($._having)),

    _having: ($) => seq($.keyword_having, $._expression),

    order_by: ($) =>
      prec.right(seq($.keyword_order, $.keyword_by, comma_list($.order_target, true))),
    ...
    parenthesized_expression: ($) => prec(2, wrapped_in_parenthesis($._expression)),
    ...
    unary_expression: ($) =>
      choice(
        ...[
          ...
          prec.left(precedence, seq(field('operator', operator), field('operand', $._expression)))
        )
      ),
    ...
    identifier: ($) => choice($._identifier, $._double_quote_string, /`([a-zA-Z_][0-9a-zA-Z_]*)`/),
